...
public class TeleportHome implements CommandExecutor {
  FileConfiguration cfg = SetHome.cfg;
    ...
    if (!(sender instanceof Player)) {
      ...
      if (!p.hasPermission("gohome.teleport")){
		...
		if (args.length == 1)  {
          ...
          if ((cfg.contains(p.getName() + "." + args[0])) && (SetHome.homes.exists()) && (SetHome.homes.length() > 1L))  {
          ...
          // Zum Home von anderem Spieler teleportieren.
        	...
        	if(p.hasPermission("gohome.teleport.tostrangers")) {
             if (cfg.contains(args[0]) && SetHome.homes.exists() && SetHome.homes.length() > 1L)
             {
